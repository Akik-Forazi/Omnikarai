...
/* MOV RAX, R12 */ static void emit_mov_rax_r12(CodeBuf*b){emit_u8(b,0x4C);emit_u8(b,0x8B);emit_u8(b,0xE0);}
/* MOV R12, RAX */ static void emit_mov_r12_rax(CodeBuf*b){emit_u8(b,0x49);emit_u8(b,0x89);emit_u8(b,0xC4);}
/* MOV RAX, R13 */ static void emit_mov_rax_r13(CodeBuf*b){emit_u8(b,0x4C);emit_u8(b,0x8B);emit_u8(b,0xE8);}
/* MOV R13, RAX */ static void emit_mov_r13_rax(CodeBuf*b){emit_u8(b,0x49);emit_u8(b,0x89);emit_u8(b,0xC5);}
/* MOV RAX, RBX */ static void emit_mov_rax_rbx(CodeBuf*b){emit_u8(b,REX_W);emit_u8(b,0x8B);emit_u8(b,0xC3);}
/* MOV RBX, RAX */ static void emit_mov_rbx_rax(CodeBuf*b){emit_u8(b,REX_W);emit_u8(b,0x89);emit_u8(b,0xC3);}
...
static int get_magic_div(int64_t d,MagicDiv*out){
    struct{int64_t d;uint64_t magic;int shift;}table[]={
        ...
        { 7,0x2492492492492493ULL,0},{ 8,0x8000000000000001ULL,2},
        {10,0xCCCCCCCCCCCCCCCDULL,3},{11,0x2E8BA2E8BA2E8BA3ULL,1},
        {13,0x4EC4EC4EC4EC4EC5ULL,1},{16,0x8000000000000001ULL,3},
        {17,0xF0F0F0F0F0F0F0F1ULL,3},{32,0x8000000000000001ULL,4},
    };
...
static void emit_mod_const(CodeBuf*b,int64_t divisor){
    // Fast path: power-of-2 divisor → AND RAX, (divisor-1)  [1 cycle]
    int k=log2_exact(divisor);
    if(k>=1){
        // AND RAX, imm  (divisor-1 = all lower k bits set)
        int64_t mask=divisor-1;
        if(mask<=127){
            emit_u8(b,REX_W);emit_u8(b,0x83);emit_u8(b,0xE0);emit_u8(b,(uint8_t)mask);
        } else {
            emit_u8(b,REX_W);emit_u8(b,0x25);emit_u32(b,(uint32_t)mask);
        }
        return;
    }
    // General: CQO + IDIV
...
static void cg_infix(CodeGen*cg,AST_Expression_Infix*node){
    ...
    // Const-RHS division: power-of-2 → SAR, otherwise load+IDIV
    if(rhs_is_const && strcmp(op,"/")==0 && rhs_const>1){
        int k=log2_exact(rhs_const);
        cg_expr(cg,node->left);
        if(k>=1){
            // SAR RAX, k  (arithmetic right shift = floor div for positive values)
...
static void cg_while_statement(CodeGen*cg,AST_Statement_While*stmt){
    ...
    for(int hi=0;hi<nhv&&n_pinned<3;hi++){
        ...
        /* Must have a valid stack slot (stack_offset > 0 means it was actually SET before) */
        if(sym->stack_offset<=0) continue;

        /* Skip variables that are SET inside the loop body — they are loop-local
           and pinning them would load stale values at loop start */
        int set_inside=0;
        for(int si=0;si<stmt->body->statement_count&&!set_inside;si++){
            AST_Statement*bs=stmt->body->statements[si];
            if(!bs)continue;
            if(bs->type==SET_STATEMENT){
                AST_Statement_Set*ss=(AST_Statement_Set*)bs;
                if(ss->name&&!strcmp(ss->name->value,hv[hi].name))set_inside=1;
            }
            /* Also check nested while/if blocks */
            if(bs->type==WHILE_STATEMENT){set_inside=1;break;} /* conservative: skip if nested loop */
            if(bs->type==IF_STATEMENT){
                AST_Statement_If*is=(AST_Statement_If*)bs;
                if(is->consequence){
                    for(int k=0;k<is->consequence->statement_count;k++){
                        AST_Statement*ks=is->consequence->statements[k];
                        if(ks&&ks->type==SET_STATEMENT&&((AST_Statement_Set*)ks)->name&&
                           !strcmp(((AST_Statement_Set*)ks)->name->value,hv[hi].name))set_inside=1;
                    }
                }
            }
        }
        if(set_inside) continue;
        ...
        /* Load current stack value into register — NO push/pop here.
           Callee-saved regs (rbx/r12/r13) are saved in the function prologue.
           If we are in the main body (not a function), we need to save them.
           Check: are we inside a function body (stack_size>32) or main? */
        if(!cg->in_main_body){
            /* Inside a function: callee-saved regs already handled by Windows x64 ABI.
               cg_fn_body will add push/pop for any slot-2,3,4 regs we use.
               For now: just load the value, the prologue/epilogue handles saves. */
        }
    ...
    /* Writeback pinned registers to stack so subsequent code sees updated values */
...
static void cg_fn_body(CodeGen*cg,AST_Statement_FnDef*fn_def){
    ...
    // Minimum stack_size=32 ensures locals are never inside the 32-byte shadow
    // space that Windows x64 callees are allowed to write to ([RSP+8..RSP+32]).
    ...
    for(int i=0;i<fn_def->parameter_count;i++){
        ...
        // If first param is 'self', record its slot so self.field works
        ...
        // Store incoming ABI registers (RCX/RDX/R8/R9) into frame
        // R8/R9 store: MOV [RBP-off], R8 = 4C 89 85 <disp32> (REX_WR, source=R8 via REX.R)
        switch(i){
