...
__attribute__((noinline)) const char* omni_os_platform(void) { return "windows"; }
...
//  SYS MODULE
// ============================================================
__attribute__((noinline)) const char* omni_sys_version(void)   { return "Omnikarai v6.0 (x86-64 Windows)"; }
...
__attribute__((noinline)) const char* omni_sys_omni_ver(void)  { return "6.0.0"; }
...
static void emit_mov_rax_r11(CodeBuf*b){emit_u8(b,REX_WB);emit_u8(b,0x8B);emit_u8(b,0xC3);}
...
static int get_magic_div(int64_t d,MagicDiv*out){
    struct{int64_t d;uint64_t magic;int shift;}table[]={
        { 2,0x8000000000000001ULL,0},{ 3,0xAAAAAAAAAAAAAAABULL,1},
        { 4,0x8000000000000001ULL,1},{ 5,0xCCCCCCCCCCCCCCCDULL,2},
        { 6,0xAAAAAAAAAAAAAAABULL,2},{ 7,0x2492492492492493ULL,0},
        { 8,0x8000000000000001ULL,2},{ 9,0x8E38E38E38E38E39ULL,1},
        {10,0xCCCCCCCCCCCCCCCDULL,3},{11,0x2E8BA2E8BA2E8BA3ULL,1},
        {12,0xAAAAAAAAAAAAAAABULL,3},{13,0x4EC4EC4EC4EC4EC5ULL,1},
        {14,0x2492492492492493ULL,1},{15,0x8888888888888889ULL,3},
        {16,0x8000000000000001ULL,3},{17,0xF0F0F0F0F0F0F0F1ULL,3},
        {18,0x8E38E38E38E38E39ULL,2},{19,0xD79435E50D79435FULL,3},
        {20,0xCCCCCCCCCCCCCCCDULL,4},{23,0xB21642C8590B2164ULL,3},
        {24,0xAAAAAAAAAAAAAAABULL,4},{25,0x47AE147AE147AE15ULL,1},
        {31,0x4210842108421085ULL,0},{32,0x8000000000000001ULL,4},
        {100,0x28F5C28F5C28F5C3ULL,5},{1000,0x20C49BA5E353F7CFULL,6},
    };
...
static void emit_mod_const(CodeBuf*b,int64_t divisor){
    // Fast path: power-of-2 → AND RAX, (divisor-1)  [1 cycle]
    ...
    // Barrett/magic-multiply path for small divisors: quotient via IMULQ magic
    // q = (rax * magic) >> (64 + shift)   then   remainder = rax - q * divisor
    // This avoids IDIV (20-90 cycles) entirely, costing ~4-6 cycles instead.
    //
    // Emit sequence:
    //   MOV  RCX, magic      ; 10 bytes
    //   MOV  R11, RAX        ; save dividend
    //   IMUL RDX:RAX, RCX    ; MUL unsigned 128-bit: result in RDX:RAX
    //     -- actually use SAR/SHR on RDX for post-shift --
    //   SHR  RDX, shift      ; keep high half
    //   IMUL RDX, divisor    ; RDX = quotient * divisor
    //   SUB  R11, RDX        ; remainder = dividend - q*divisor
    //   MOV  RAX, R11
    //
    // For the magic table we use unsigned multiply (MUL RCX) on RDX:RAX.
    MagicDiv md;
    if(get_magic_div(divisor,&md)){
        // Save dividend in R11
        emit_u8(b,0x49);emit_u8(b,0x89);emit_u8(b,0xC3); // MOV R11, RAX
        // MOV RCX, magic (imm64)
        ...
        // SHR RDX, post_shift  (if post_shift > 0)
        if(md.post_shift>0){
            if(md.post_shift==1){emit_u8(b,REX_W);emit_u8(b,0xD1);emit_u8(b,0xEA);}
            else{emit_u8(b,REX_W);emit_u8(b,0xC1);emit_u8(b,0xEA);emit_u8(b,(uint8_t)md.post_shift);}
        }
        // IMUL RDX, divisor  →  quotient * divisor
        if(divisor>=-128&&divisor<=127){
            emit_u8(b,REX_W);emit_u8(b,0x6B);emit_u8(b,0xD2);emit_u8(b,(uint8_t)(int8_t)divisor);
        } else {
            emit_u8(b,REX_W);emit_u8(b,0x69);emit_u8(b,0xD2);emit_u32(b,(uint32_t)(int32_t)divisor);
        }
        // SUB R11, RDX  →  remainder
        emit_u8(b,0x49);emit_u8(b,0x29);emit_u8(b,0xD3); // SUB R11, RDX
        // MOV RAX, R11
        emit_u8(b,REX_WB);emit_u8(b,0x8B);emit_u8(b,0xC3); // MOV RAX, R11
        return;
    }
    // General fallback: CQO + IDIV (only for divisors not in table)
    emit_u8(b,REX_W);emit_u8(b,0xB9);emit_u64(b,(uint64_t)divisor);
    emit_u8(b,REX_W);emit_u8(b,0x99);
    emit_u8(b,REX_W);emit_u8(b,0xF7);emit_u8(b,0xF9);
    emit_u8(b,REX_W);emit_u8(b,0x89);emit_u8(b,0xD0);
}
...
static void cg_infix(CodeGen*cg,AST_Expression_Infix*node){
    ...
    // Const-RHS division: power-of-2 → SAR; magic table → Barrett; else IDIV
    if(rhs_is_const && strcmp(op,"/")==0 && rhs_const>1){
        int k=log2_exact(rhs_const);
        cg_expr(cg,node->left);
        if(k>=1){
            ...
            MagicDiv md2;
            if(get_magic_div(rhs_const,&md2)){
                emit_u8(&cg->code,0x49);emit_u8(&cg->code,0x89);emit_u8(&cg->code,0xC3); // MOV R11,RAX (save)
                emit_u8(&cg->code,REX_W);emit_u8(&cg->code,0xB9);emit_u64(&cg->code,md2.magic);
                emit_u8(&cg->code,REX_W);emit_u8(&cg->code,0xF7);emit_u8(&cg->code,0xE1); // MUL RCX
                if(md2.post_shift>0){
                    if(md2.post_shift==1){emit_u8(&cg->code,REX_W);emit_u8(&cg->code,0xD1);emit_u8(&cg->code,0xEA);}
                    else{emit_u8(&cg->code,REX_W);emit_u8(&cg->code,0xC1);emit_u8(&cg->code,0xEA);emit_u8(&cg->code,(uint8_t)md2.post_shift);}
                }
                emit_u8(&cg->code,REX_W);emit_u8(&cg->code,0x89);emit_u8(&cg->code,0xD0); // MOV RAX,RDX (quotient)
            } else {
        ...
        case CALL_EXPRESSION:{
            ...
            if(c->function&&c->function->type==MEMBER_ACCESS_EXPRESSION){
                ...
                if(strcmp(ma->member,"format")==0||strcmp(ma->member,"format_date")==0||
                   ...
                   strcmp(ma->member,"fromint")==0)return OMNI_TYPE_STR;
...
static void cg_call_print(CodeGen*cg,AST_Expression_Call*node){
    ...
    cg_expr(cg,arg);emit_mov_rcx_rax(&cg->code);
...
static int pkg_load_into_cg(CodeGen* cg, const char* pkg_name) {
    ...
    char search[1100];
    snprintf(search, sizeof(search), "%s\\*.ok", pkg_dir);
    WIN32_FIND_DATAA fd;
    ...
    do {
        ...
        char filepath[1200];
        snprintf(filepath, sizeof(filepath), "%s\\%s", pkg_dir, fd.cFileName);
...
static void cg_module_call(CodeGen*cg,AST_Expression_Call*call,const char*ns,const char*method){
    ...
    if(strcmp(ns,"ai")==0){
        ...
        if(strcmp(method,"matmul")==0&&argc==5){
            ...
            // rsp = rbp - aligned_frame(stack_size). s4 >= stack_size so
            // s4 > aligned_frame-32 meaning [rbp-s4] is safely below [rsp+32].
            // We write via rsp+32 which is the correct ABI slot.
    ...
    if (pkg_is_loaded(ns)) {
        ...
        if (pe) {
            ...
            if (argc >= 1) emit_load_rcx(&cg->code, arg_offsets[0]);
            if (argc >= 2) emit_load_rdx(&cg->code, arg_offsets[1]);
            if (argc >= 3) emit_load_r8 (&cg->code, arg_offsets[2]);
            if (argc >= 4) emit_load_r9 (&cg->code, arg_offsets[3]);
...
static void cg_call_user(CodeGen*cg,AST_Expression_Call*node,const char*fn_name){
    ...
    if(ce){
        ...
        if(init_fe){
            ...
            if(total>=1)emit_load_rcx(&cg->code,slots[0]);
            if(total>=2)emit_load_rdx(&cg->code,slots[1]);
            if(total>=3)emit_load_r8(&cg->code,slots[2]);
            if(total>=4)emit_load_r9(&cg->code,slots[3]);
            ...
            if(call->function&&call->function->type==MEMBER_ACCESS_EXPRESSION){
                ...
                if(ma->object&&ma->object->type==IDENTIFIER){
                    ...
                    if(ce){
                        ...
                        if(total_args>=1)emit_load_rcx(&cg->code,slots[0]);
                        if(total_args>=2)emit_load_rdx(&cg->code,slots[1]);
                        if(total_args>=3)emit_load_r8(&cg->code,slots[2]);
                        if(total_args>=4)emit_load_r9(&cg->code,slots[3]);
                        cg->scope->next_offset=base;
                        // Emit call patch
                        emit_u8(&cg->code,0xE8);size_t pp=cg->code.size;emit_u32(&cg->code,0);
                        if(cg->call_patch_count>=MAX_CALL_PATCHES){fprintf(stderr,"Fatal: too many call patches\n");exit(1);}
...
static void cg_for_range(CodeGen*cg,AST_Statement_For*stmt,AST_Expression_Call*range_call){
    ...
    if(use_reg){
        ...
        if(use_reg){
            ...
            int fbody_saves[3]; int fbody_nsaved=0;
            {
            ...
            for(int sidx=fbody_nsaved-1;sidx>=0;sidx--){
                ...
                if(fs2){switch(fslot){case 2:emit_mov_rax_rbx(&cg->code);break;case 3:emit_mov_rax_r12(&cg->code);break;case 4:emit_mov_rax_r13(&cg->code);break;} emit_store_rax(&cg->code,fs2->stack_offset);}
...
static int is_known_module(const char* name){
    ...
    /* 2. installed packages:
          %LOCALAPPDATA%\Programs\omnikarai\site-packages\<name>\
       Falls back to %USERPROFILE%\AppData\Local\... if LOCALAPPDATA not set. */
    const char* lad = getenv("LOCALAPPDATA");
    if(!lad){
        const char* home = getenv("USERPROFILE");
        if(!home) return 0;
...
static void cg_fn_body(CodeGen*cg,AST_Statement_FnDef*fn_def){
    ...
    /* ── REGISTER ALLOCATOR ─────────────────────────────────────────────────
       Reserve [rbp-8],[rbp-16],[rbp-24] for callee-saved regs rbx,r12,r13.
       Variables (params + locals) start at offset 32, not 8.
       Frame: [rbp-8]=rbx [rbp-16]=r12 [rbp-24]=r13 [rbp-32..]=params+locals
       Analyze body to find 3 hottest local variables, pin them to registers.
       This eliminates stack load/store for inner loop vars, matching C speed. */
    fn_scope->next_offset = 32;  /* skip callee-save zone */
    ...
    /* Analyze body: count reads+writes per variable */
    HotVar ra_hv[HOT_MAX]; int ra_nhv=0;
    count_block_reads(fn_def->body, ra_hv, &ra_nhv);
    /* Sort by count DESC (insertion sort) */
    for(int i=1;i<ra_nhv;i++){HotVar tmp=ra_hv[i];int j=i-1;while(j>=0&&ra_hv[j].count<tmp.count){ra_hv[j+1]=ra_hv[j];j--;}ra_hv[j+1]=tmp;}

    /* Pin top variables to rbx(slot2)/r12(slot3)/r13(slot4).
       Skip params — they get ABI registers anyway.
       Skip vars with count < 4 — not worth the register pressure. */
    int ra_pinned=0;
    for(int ri=0;ri<ra_nhv&&ra_pinned<3;ri++){
        ...
        /* assign slots 2,3,4 = rbx,r12,r13.
           reg_var_depth is set to 2+ra_pinned so cg_identifier loop
           0..reg_var_depth covers slots 0,1 (r14/r15 empty) + 2,3,4 (our regs) */
        strncpy_s(cg->reg_var_names[2+ra_pinned],64,ra_hv[ri].name,_TRUNCATE);
        ra_pinned++;
    }
    /* Set depth to 2+ra_pinned: slots 0,1 are empty strings (r14/r15 not used here)
       slots 2..2+ra_pinned-1 hold the pinned variable names */
    cg->reg_var_depth = 2 + ra_pinned;
    if(g_beta||1){fprintf(stderr,"[REGALLOC] fn='%s' pinned=%d",fn_def->name->value,ra_pinned);for(int _d=0;_d<ra_pinned;_d++)fprintf(stderr," slot%d='%s'",2+_d,cg->reg_var_names[2+_d]);fprintf(stderr,"\n");}

    /* Emit callee-save for used registers */
    /* MOV [rbp-8], RBX  */ if(ra_pinned>=1){emit_u8(&cg->code,0x48);emit_u8(&cg->code,0x89);emit_u8(&cg->code,0x5D);emit_u8(&cg->code,0xF8);}
    /* MOV [rbp-16], R12 */ if(ra_pinned>=2){emit_u8(&cg->code,0x4C);emit_u8(&cg->code,0x89);emit_u8(&cg->code,0x65);emit_u8(&cg->code,0xF0);}
    /* MOV [rbp-24], R13 */ if(ra_pinned>=3){emit_u8(&cg->code,0x4C);emit_u8(&cg->code,0x89);emit_u8(&cg->code,0x6D);emit_u8(&cg->code,0xE8);}

    /* Store parameters from ABI regs into frame */
    ...
    /* Emit epilogue: restore callee-saved regs, then return */
    if(!cg->returned){
        emit_xor_rax_rax(&cg->code);
        /* MOV R13, [rbp-24] */ if(ra_pinned>=3){emit_u8(&cg->code,0x4C);emit_u8(&cg->code,0x8B);emit_u8(&cg->code,0x6D);emit_u8(&cg->code,0xE8);} /* 3 pinned */
        /* MOV R12, [rbp-16] */ if(ra_pinned>=2){emit_u8(&cg->code,0x4C);emit_u8(&cg->code,0x8B);emit_u8(&cg->code,0x65);emit_u8(&cg->code,0xF0);} /* 2+ pinned */
        /* MOV RBX, [rbp-8]  */ if(ra_pinned>=1){emit_u8(&cg->code,0x48);emit_u8(&cg->code,0x8B);emit_u8(&cg->code,0x5D);emit_u8(&cg->code,0xF8);} /* 1+ pinned */
...
int64_t codegen_run(CodeGen*cg){
    ...
    void*mem=VirtualAlloc(NULL,cg->code.size,MEM_COMMIT|MEM_RESERVE,PAGE_EXECUTE_READWRITE);
    if(!mem){fprintf(stderr,"CodeGen: VirtualAlloc failed (%lu)\n",GetLastError());return -1;}
